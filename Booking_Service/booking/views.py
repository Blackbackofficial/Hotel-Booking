...
def create_or_all(request):
    ...
    try:
        ...
        if request.method == 'POST':
            payBalance = requests.post("https://hotels-payment-chernov.herokuapp.com/api/v1/payment/create",
            ...
            for res in users_reservations:
                payBalance = requests.get(
                    "https://hotels-payment-chernov.herokuapp.com/api/v1/payment/status/{}".format(res['fields'].get("payment_uid")),
                    cookies=request.COOKIES)
                if payBalance.status_code == 200:
                    payBalance = payBalance.json()
                    res['fields'].update(payBalance)
                hotel = requests.get(
                    "https://hotels-payment-chernov.herokuapp.com/api/v1/hotel/status/{}".format(res['fields'].get("hotel_uid")),
                    cookies=request.COOKIES)
...
def canceled(request, booking_uid):
    try:
        ...
        payStatus = requests.delete("https://hotels-payment-chernov.herokuapp.com/api/v1/payment/close/{}".format(payment_uid),
                                    cookies=request.COOKIES)
...
def pay(request, booking_uid):
    try:
        auth(request)
        payment_uid = Reservations.objects.get(booking_uid=booking_uid).payment_uid
        payStatus = requests.post("https://hotels-payment-chernov.herokuapp.com/api/v1/payment/pay/{}".format(payment_uid),
                                  cookies=request.COOKIES)
        if payStatus.status_code == 200:
            return JsonResponse(payStatus.json(), status=status.HTTP_200_OK)
...
def reversed(request, booking_uid):
    try:
        auth(request)
        payment_uid = Reservations.objects.get(booking_uid=booking_uid).payment_uid
        payStatus = requests.post("https://hotels-payment-chernov.herokuapp.com/api/v1/payment/reversed/{}".format(payment_uid),
                                  cookies=request.COOKIES)
        if payStatus.status_code == 200:
            return JsonResponse(payStatus.json(), status=status.HTTP_200_OK)
...
def about_one(request, booking_uid):
    try:
        ...
        hotel = requests.get("https://hotels-payment-chernov.herokuapp.com/api/v1/hotel/status/{}".format(reservations["hotel_uid"]),
                             cookies=request.COOKIES)  # нужно доделать
        if hotel.status_code == 200:
            hotel = hotel.json()
            reservations.update(hotel)
        payBalance = requests.get("https://hotels-payment-chernov.herokuapp.com/api/v1/payment/status/{}".format(reservations["payment_uid"]),
                                  cookies=request.COOKIES)
        if payBalance.status_code == 200:
            payBalance = payBalance.json()
...
def filter_booking(request, date_start, date_end):
    try:
        ...
        for res in reservations:
            payBalance = requests.get(
                "https://hotels-payment-chernov.herokuapp.com/api/v1/payment/status/{}".format(res["payment_uid"]),
                cookies=request.COOKIES)
            if payBalance.status_code == 200:
                payBalance = payBalance.json()
    ...
    try:
        ...
        for res in reservations:
            ...
            about_hotel = requests.get(
                "https://hotels-hotel-chernov.herokuapp.com/api/v1/hotels/{}".format(res['fields'].get("hotel_uid")),
                cookies=request.COOKIES)
            if about_hotel.status_code == 200:
                about_hotel = about_hotel.json()
                res['fields'].update(about_hotel)
            user = requests.get(
                "https://hotels-session-chernov.herokuapp.com/api/v1/session/user/{}".format(res['fields'].get("user_uid")),
                cookies=request.COOKIES)
            if user.status_code == 200:
                user = user.json()
                res['fields'].update(user)
            loyalty = requests.get(
                "https://hotels-loyalty-chernov.herokuapp.com/api/v1/loyalty/status/{}".format(res['fields'].get("user_uid")),
                cookies=request.COOKIES)
